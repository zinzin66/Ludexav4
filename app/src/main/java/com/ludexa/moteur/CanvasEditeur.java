...
public class CanvasEditeur extends View {
    ...
    // Système d'état complet pour les interactions
    private int currentMode = 0; 
    // 0: NONE, 1: PAN, 2: MOVE, 4: RESIZE_TL, 5: RESIZE_TR, 6: RESIZE_BL, 7: RESIZE_BR, 8: ROTATE
    
    private float dragStartX, dragStartY;
    
    // Variables pour mémoriser l'état initial de l'objet lors d'une interaction
    ...
    private void init() {
        ...
        // Paint pour les poignées interactives
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (sceneActive != null) {
            for (ObjetBase objet : sceneActive.objets) {
                ...
                // Application de la rotation pour le dessin
                canvas.save();
                if (objet.rotation != 0) {
                ...
                if (objet == objetSelectionne) {
                    ...
                    float hs = 12f;
                    canvas.drawRect(left - 4 - hs, top - 4 - hs, left - 4 + hs, top - 4 + hs, paintPoignee); // TL
                    canvas.drawRect(right + 4 - hs, top - 4 - hs, right + 4 + hs, top - 4 + hs, paintPoignee); // TR
                    canvas.drawRect(left - 4 - hs, bottom + 4 - hs, left - 4 + hs, bottom + 4 + hs, paintPoignee); // BL
                    canvas.drawRect(right + 4 - hs, bottom + 4 - hs, right + 4 + hs, bottom + 4 + hs, paintPoignee); // BR
    ...
    // Convertit un point de la scène vers l'espace local d'un objet
    ...
    // Récupère la position d'un point local dans la scène
    ...
    private int getTouchTarget(float xEcran, float yEcran) {
        ...
        if (objetSelectionne != null) {
            ...
            float rotY = top - 4 - 50f;
            if (Math.hypot(lx - midX, ly - rotY) < hit) return 8; // ROTATE
            
            if (Math.abs(lx - left) < hit && Math.abs(ly - top) < hit) return 4; // TL
            if (Math.abs(lx - right) < hit && Math.abs(ly - top) < hit) return 5; // TR
            if (Math.abs(lx - left) < hit && Math.abs(ly - bottom) < hit) return 6; // BL
            if (Math.abs(lx - right) < hit && Math.abs(ly - bottom) < hit) return 7; // BR
            
            if (lx >= left && lx <= right && ly >= top && ly <= bottom) return 2; // MOVE
        }
